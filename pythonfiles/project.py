...
def changepasswd(user_id):
    ...
    Label(text='new password').place(x=500,y=100)
    Entry(textvariable=a1,show='•',width=60).place(x=500,y=130)
    Label(text='new password').place(x=500,y=200)
    Entry(textvariable=b1,show='•',width=60).place(x=500,y=230)
    u.CTkButton(window,bg_color='white',fg_color='black',text='submit',command=lambda:checnewpass(user_id)).place(x=500,y=300)
def profile(user_id):
    ...
    profile_canvas.place(x=1000, y=260)
    ...
    profile_canvas.create_image(0, 0, anchor=NW, image=profile_photo)
    profile_canvas.image = profile_photo
    Label(text=f'{user_id[0][2]}',font=("Bradley Hand ITC",40),bg='white').place(x=800,y=280)
...
def show_seat_selection(user_id,b,busdet,places,cost):#shows the seats selected by the user and allows him to select
    ...
    u.CTkButton(window,text='click me',command=lambda :bookingpage(user_id,b,busdet,places,cost)
                ...
                ,font=("Arial Bold",16)).place(x=560,y=650)
def pdf_gen(user_id,bus_id,busdet,places,cost):
    ...
    departure_time = str(busdet[1])
    arrival_time =str( busdet[2])
    ...
    image_path =  parentDirectory+'image//buslogo.png'
    image_x = 400
    image_y = 720
    image_width = 100
    image_height = 100
    ...
    pdf.drawString(text_x, image_y - 160, "Fare: " + fare)
    pdf.drawString(text_x, image_y - 180, "Distance: " + distance)
    pdf.drawString(text_x, image_y - 200, "Seats Booked: " + seats_booked)
    pdf.drawString(text_x, image_y - 220, "Total Cost: " + fare)
...
def bookingpage(user_id,bus_id,busdet,places,cost):#gui page to nook seats
    ...
    Label(bg='white',font=("Comic Sans MS ",16),text=cost).place(x=300,y=560)
